...
Citizen.CreateThread(function()
    ...
    getPlayerJobFromServer(function(job)
        ...
        print("[Az-FR | Core System] Access granted for job: " .. tostring(job))
            ...
            -- New constants for H hold force-end
            local FORCE_HOLD_MS = 5000  -- hold H for 5000ms (5s) to force-end
            local hHoldStart = nil

            local contextOpenCount = 0

            local blockedControlsWhileMenu = {
                38,   -- E
                47,   -- G
                74,   -- H
                249,  -- N
            ...
            -- Centralized local cleanup helper
                ...
                if type(lib) == "table" then
                    ...
                    if type(lib.showContext) == "function" and not lib.__wrapped_showContext then
                        ...
                        lib.showContext = function(id, ...)
                            ...
                            pcall(function() log("lib.showContext called id=%s count=%d", tostring(id), contextOpenCount) end)
                    ...
                    if type(lib.hideContext) == "function" and not lib.__wrapped_hideContext then
                        ...
                        lib.hideContext = function(force, ...)
                            ...
                            pcall(function() log("lib.hideContext called count=%d", contextOpenCount) end)
            ...
            local function hasAssignedCallout()
                for k, v in pairs(assignedCalloutsToMe) do
                    if v then
                        return true
                    end
                end
            ...
            -- Modified acceptCallout: mark locally accepted immediately and set assigned flag
            local function acceptCallout(idstr)
                ...
                -- Immediately mark locally accepted so the local expiry won't remove the callout
            ...
            AddEventHandler("callouts:new", function(smallInst)
                ...
                -- preserve accepted state if already set; otherwise false
            ...
            AddEventHandler("callouts:accepted", function(payload)
                ...
                -- ensure accepted flag is set (server-confirmed)
                active[idstr].accepted = true
                ...
                if payload.assignedTo and tostring(payload.assignedTo) == tostring(myServerId) then
                    ...
                    -- make sure to clear our local assigned flag if server assigned someone else
                    assignedCalloutsToMe[idstr] = nil
            ...
            AddEventHandler("callouts:action_failed", function(calloutId, reason)
                ...
                if tostring(reason) == "NOT_FOUND" then
                    ...
                    if localEntry and localEntry.data then
                        ...
                        log("CLIENT fallback: server NOT_FOUND for id=%s; requesting server create+assign from local data (template=%s)", idstr, tostring(smallInst.template))
            ...
            -- Helper to get first assigned callout id
            ...
            RegisterCommand("callout_menu", function(source, args)
                ...
                -- If no arg and user has an assigned callout, open that callout's menu
                if not idarg then
            ...
            Citizen.CreateThread(function()
                while true do
                    ...
                    -- E (Accept / Status quick reply)
                    if IsControlJustReleased(0, 38) then
                        if menuOpen then goto continue_e end
                    ...
                    if IsControlJustReleased(0, 47) then
                        if menuOpen then goto continue_g end
                    ...
                    -- H key: supports short-press (end if near) and long-hold to force-end
                    ...
                    if IsControlJustReleased(0, 74) then
                        local heldFor = nil
                        if hHoldStart then heldFor = GetGameTimer() - hHoldStart end
                        hHoldStart = nil

                        if heldFor and heldFor >= FORCE_HOLD_MS then
                            -- force-end all assigned callouts locally and notify server
                            for id, _ in pairs(assignedCalloutsToMe) do
                                if assignedCalloutsToMe[id] then
                                    log("CLIENT H held: force ending id=%s", id)
                                    cleanupLocalCallout(id)
                                    doNotify({ id = "callout_force_end_" .. id, title = "Callout", description = "Force-ended callout " .. tostring(id), type = "warning", duration = 4000 })
                                    TriggerServerEvent("callouts:end", id) -- inform server as well
                                end
                            end
                        else
                            -- short press: original behavior (end if near scene)
                            for id, _ in pairs(assignedCalloutsToMe) do
                                if distToCallout(id) <= END_DISTANCE_THRESHOLD then
                                    ...
                                    doNotify({ id = "callout_end_too_far", title = "Callout", description = "You are too far from callout " .. tostring(id) .. " to end it.", type = "warning", duration = 3000 })
                                end
                            end
                        end
                    end
                    ...
                    -- Vehicle info display when holding sprint (LEFT SHIFT)
                ...
                if not cfg then
                    ...
                    elseif not minT or not maxT or minT <= 0 or maxT < minT then
                        ...
                        local function loadTemplatesFromManifest()
                            ...
                            if #parsed > 0 then
                                for i, v in ipairs(parsed) do
                                    if type(v) == "string" then table.insert(arr, v) end
                                end
                            else
                                ...
                                elseif parsed["files"] and type(parsed["files"]) == "table" then
                                    ...
                                    for k, v in pairs(parsed) do
                                        if type(k) == "number" and type(v) == "string" then table.insert(arr, v) end
                                    end
                                end
                            end
                        ...
                        local function titleCase(s)
                            return s:gsub("(%S+)", function(word) return word:sub(1,1):upper() .. (word:sub(2) or "") end)
                        end
                        ...
                        local function rangeToMs(minv, maxv, unitv)
                            ...
                            if rmax == rmin then return rmin * mult end
                        ...
                        local function pickSpawnCoords()
                            ...
                            local nz = pz
                            if found and gz then nz = gz end
            ...
            RegisterNetEvent("callouts:edit_record")
            AddEventHandler("callouts:edit_record", function() end) 
            ...
            print("[Az-FR | Core System] You are not an allowed department.")
        end
    end)
end)
