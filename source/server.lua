...
AddEventHandler("mdt:logID", function(netIdStr, identity)
    ...
    pcall(function()
        logID(netIdStr, displayName, (identity and identity.name) or "", "ID Created")
    end)

    TriggerClientEvent("mdt:idResult", src, {
        netId = netIdStr,
        name = (identity and identity.name) or "",
        dob = (identity and identity.dob) or "",
...
AddEventHandler("police:arrestPed", function(netId, fullName, dob)
    local src = source
    local ids = GetPlayerIdentifiers(src)
    local identifier = ids and ids[1] or ("player:" .. tostring(src))
    local displayName = GetPlayerName(src) or identifier
...
AddEventHandler("mdt:lookupPlate", function(plate, fullName, make, color)
    local src = source
    if not plate then
...
AddEventHandler("mdt:listRecords", function(data)
    ...
    if not data or not data.target_type or not data.target_value then
        TriggerClientEvent("mdt:recordsResult", src, {}, data and data.target_type or nil, data and data.target_value or nil)
        return
    end
...
AddEventHandler("mdt:ping", function()
    print(("SERVER DEBUG: got mdt:ping from source=%s"):format(tostring(source)))
    TriggerClientEvent("mdt:pingResult", source, true)
end)
